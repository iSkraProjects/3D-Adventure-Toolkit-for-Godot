@tool
class_name ATKConditionMissingItem
...
func _evaluate_internal(context: Dictionary) -> Dictionary:
	if item_id.is_empty():
		return fail("Missing-item condition '%s' is missing item_id." % get_debug_name())

	var inventory := _resolve_inventory(context)
	if inventory == null:
		return fail(
			"Missing-item condition '%s' could not find ATKInventory." % get_debug_name(),
			{"item_id": item_id, "amount": amount}
		)

	var required_amount := maxi(amount, 1)
	var missing: bool = not inventory.has_item(item_id, required_amount)
	if missing:
		return succeed(
			"Inventory does not contain '%s' x%d." % [item_id, required_amount],
